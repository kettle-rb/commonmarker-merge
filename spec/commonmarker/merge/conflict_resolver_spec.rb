...
RSpec.describe Commonmarker::Merge::ConflictResolver do
  ...
  it_behaves_like "Ast::Merge::ConflictResolverBase" do
    ...
    let(:build_mock_analysis) do
      -> {
        source = "# Mock Content\n"
        Commonmarker::Merge::FileAnalysis.new(source)
      }
    end
  end
  ...
  describe "#resolve" do
    ...
    context "with identical content" do
      ...
      let(:template_analysis) { Commonmarker::Merge::FileAnalysis.new(identical_source) }
      let(:dest_analysis) { Commonmarker::Merge::FileAnalysis.new(identical_source) }
    ...
    context "with frozen destination node" do
      let(:dest_source) do
        <<~MARKDOWN
          <!-- commonmarker-merge:freeze Custom reason -->
          ## Frozen Section
      ...
      it "returns frozen decision" do
        expect(freeze_node).to be_a(Commonmarker::Merge::FreezeNode)
    ...
    context "with frozen template node" do
      ...
      it "returns frozen decision" do
        expect(template_freeze_node).to be_a(Commonmarker::Merge::FreezeNode)
  ...
  describe "#node_to_text" do
    ...
    context "with FreezeNode" do
      ...
      let(:freeze_analysis) { Commonmarker::Merge::FileAnalysis.new(freeze_source) }
    ...
    context "with node without source_position" do
      it "falls back to to_commonmark" do
        ...
        allow(node).to receive(:is_a?).with(Commonmarker::Merge::FreezeNode).and_return(false)
