...
RSpec.describe Commonmarker::Merge::FreezeNode do
  # Note: We include only specific shared examples rather than the full "Ast::Merge::FreezeNodeBase"
  # shared example group, because Commonmarker::Merge::FreezeNode has a custom signature format
  ...
  let(:build_freeze_node) do
    ->(start_line:, end_line:, **opts) {
      ...
      # Note: Commonmarker::Merge::FreezeNode hard-codes pattern_type to :html_comment internally
      # so we don't pass it to the constructor
      freeze_node_class.new(
        start_line: start_line,
        end_line: end_line,
        content: content,
        start_marker: opts[:start_marker] || "<!-- commonmarker-merge:freeze -->",
        end_marker: opts[:end_marker] || "<!-- commonmarker-merge:unfreeze -->",
        nodes: opts[:nodes] || [],
        reason: opts[:reason],
      )
    }
  ...
  # Shared contract tests (subset that applies to commonmarker's implementation)
  describe "instance methods (shared contract)" do
    ...
    describe "#signature" do
      ...
      # Commonmarker uses :freeze_block (not :FreezeNodeBase) as a content-based signature
  ...
  describe ".pattern_for" do
    context "without token (returns Hash)" do
      ...
      it "has working start pattern" do
        pattern = described_class.pattern_for(:html_comment)
        expect(pattern[:start]).to match("<!-- commonmarker-merge:freeze -->")
      end

      it "has working end pattern" do
        pattern = described_class.pattern_for(:html_comment)
        expect(pattern[:end]).to match("<!-- commonmarker-merge:unfreeze -->")
      end
    end

    context "with token (returns Regexp)" do
      it "builds html_comment pattern with default token" do
        pattern = described_class.pattern_for(:html_comment, "commonmarker-merge")
        expect(pattern).to be_a(Regexp)
        expect(pattern).to match("<!-- commonmarker-merge:freeze -->")
        expect(pattern).to match("<!-- commonmarker-merge:unfreeze -->")
      end
      ...
      it "builds html_comment pattern with custom token" do
        ...
        expect(pattern).not_to match("<!-- commonmarker-merge:freeze -->")
      end
      ...
      it "captures marker type" do
        pattern = described_class.pattern_for(:html_comment, "commonmarker-merge")
        match = "<!-- commonmarker-merge:freeze -->".match(pattern)
        expect(match[1]).to eq("freeze")

        match = "<!-- commonmarker-merge:unfreeze -->".match(pattern)
        expect(match[1]).to eq("unfreeze")
      end

      it "captures optional reason" do
        pattern = described_class.pattern_for(:html_comment, "commonmarker-merge")
        match = "<!-- commonmarker-merge:freeze My Reason -->".match(pattern)
  ...
  describe "#initialize" do
    let(:freeze_node) do
      described_class.new(
        ...
        start_marker: "<!-- commonmarker-merge:freeze -->",
        end_marker: "<!-- commonmarker-merge:unfreeze -->",
        nodes: [],
      )
    end
    ...
    it "stores start_marker" do
      expect(freeze_node.start_marker).to eq("<!-- commonmarker-merge:freeze -->")
    end

    it "stores end_marker" do
      expect(freeze_node.end_marker).to eq("<!-- commonmarker-merge:unfreeze -->")
    end
    ...
    context "with reason in marker" do
      let(:freeze_node) do
        described_class.new(
          ...
          end_line: 3,
          content: "content",
          start_marker: "<!-- commonmarker-merge:freeze Custom Reason -->",
          end_marker: "<!-- commonmarker-merge:unfreeze -->",
        )
      end
  ...
  describe "#contains_type?" do
    let(:mock_heading) { double("Node", type: :heading) }
    ...
    let(:freeze_node) do
      described_class.new(
        ...
        start_marker: "<!-- m:freeze -->",
        end_marker: "<!-- m:unfreeze -->",
        nodes: [mock_heading, mock_paragraph],
      )
    end

    it "returns true for contained type" do
      expect(freeze_node.contains_type?(:heading)).to be true
  ...
  describe "#reason extraction via base class pattern_for" do
    it "extracts reason when present" do
      node = described_class.new(
        ...
        end_line: 3,
        content: "content",
        start_marker: "<!-- commonmarker-merge:freeze Custom Reason Here -->",
        end_marker: "<!-- commonmarker-merge:unfreeze -->",
      )
    ...
    it "handles reason with special characters" do
      node = described_class.new(
        ...
        end_line: 3,
        content: "content",
        start_marker: "<!-- commonmarker-merge:freeze Keep this: important! -->",
        end_marker: "<!-- commonmarker-merge:unfreeze -->",
      )
    ...
    it "extracts reason even when it starts with dash (base class behavior)" do
      ...
      node = described_class.new(
        ...
        end_line: 3,
        content: "content",
        start_marker: "<!-- commonmarker-merge:freeze -invalid -->",
        end_marker: "<!-- commonmarker-merge:unfreeze -->",
      )
