...
RSpec.describe Commonmarker::Merge::DebugLogger do
  describe "module configuration" do
    it "has env_var_name set" do
      expect(described_class.env_var_name).to eq("COMMONMARKER_MERGE_DEBUG")
    end

    it "has log_prefix set" do
      expect(described_class.log_prefix).to eq("[commonmarker-merge]")
    end
  end
  ...
  describe ".debug" do
    context "when debug disabled" do
      before do
        allow(ENV).to receive(:[]).with("COMMONMARKER_MERGE_DEBUG").and_return(nil)
      end
    ...
    context "when debug enabled" do
      before do
        allow(ENV).to receive(:[]).with("COMMONMARKER_MERGE_DEBUG").and_return("1")
      end
      ...
      it "includes prefix" do
        expect { described_class.debug("test") }.to output(/\[commonmarker-merge\]/).to_stderr
      end
    end
  end
